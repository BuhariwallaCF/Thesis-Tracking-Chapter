...
tags.df$depl_date <- tags.df$date
...
tags.df <- merge(tags.df, tag.end, by = "id")
...
long.tags <- c("33160", "33161", "33158", "33162", "33159")
 ...
 for(i in 1:length(tags.df$id)){
  if(tags.df$id[i] %in% long.tags){tags.df$end_date[i] <- ymd_hms("2015-05-26 00:00:01")}}

tags.df$tag_int <- new_interval(tags.df$depl_date, tags.df$end_date)
tags.df$days_active <- tags.df$end_date - tags.df$depl_date
...
remove.tags.df <- NULL
for(i in tag.df$id){
  temp <- mr.df[mr.df$id == i & mr.df$date < tags.df$depl_date[tags.df$id == i],]  
