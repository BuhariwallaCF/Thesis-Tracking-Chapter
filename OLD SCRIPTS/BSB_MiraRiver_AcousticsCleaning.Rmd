...
tag.end <- mrdf %>% group_by(id) %>% select(id, date) %>% summarise(end_date = max(date))
...
long.tags <- c("33160", "33161", "33158", "33162", "33159")
 ...
 for(i in 1:length(tagdf$id)){
  if(tagdf$id[i] %in% long.tags){tagdf$end_date[i] <- ymd_hms("2015-05-26 00:00:01")}}
```

```{r}

tagdf$tag_int <- new_interval(tagdf$depl_date, tagdf$end_date)
tagdf$days_active <- tagdf$end_date - tagdf$depl_date
## station  - use the station metadata to create an interval of deployment and recovery 
stndf$station_int <- new_interval(stndf$depl_date, stndf$recov_date)
stndf$days_active <- stndf$recov_date - stndf$depl_date
## ZONE
zonedf$zone_int <- new_interval(zonedf$depl_date, zonedf$recov_date)
```
