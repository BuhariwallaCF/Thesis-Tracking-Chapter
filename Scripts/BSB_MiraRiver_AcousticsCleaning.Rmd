...
Add River Zone* This will not work since the stations are in character
```{r}
...
system( 
  for(i in 1:length(stndf$sn)){
  temp <- mrdf[mrdf$receiver == stndf$sn[i] & mrdf$date %within% stndf$activity_interval[stndf$sn == stndf$sn[i]] == FALSE & mrdf$station != stndf$station[i],]
stn.check2 <- rbind(stn.check2, temp)
}
)
